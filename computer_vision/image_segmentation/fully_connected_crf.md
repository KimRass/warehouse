...
$$\theta _{ij} (x_{i}, x_{j}) = \mu (x_{i}, x_{j}) \big[ \omega _{1} \exp \Big( - \frac{\lVert p_{i} - p_{j} \rVert ^{2}}{2 \sigma _{\alpha} ^{2}} - \frac{\lVert I_{i} - I_{j} \rVert ^{2}}{2 \sigma _{\beta} ^{2}} \Big) + \omega _{2} \exp \Big( - \frac{\lVert p_{i} - p_{j} \rVert ^{2}}{2 \sigma _{\gamma} ^{2}} \Big) \big]$$
